...
import { runStreetEngineV3Preparation, type StreetEngineV3PreparationOptions } from './v3Preparation.ts';
...
export type PreparationRunnerOptions = AreaTaskPreparationOptions & StreetEngineV3PreparationOptions;
...
export class PreparationRunner {
  ...
  async alarm() {
    ...
    // Arm crash recovery before any source-pack or legacy source work. A killed
    // invocation leaves a durable wake-up and the generation remains pinned.
    await this.storage.setAlarm(Date.now()+65000);
    {
      ...
      if(this.options.streetEngineVersion==='sourcepack-v3')await runStreetEngineV3Preparation(db,run,this.options);

...
import { applyNetworkCoverage } from '../../src/domain/streetNetwork.ts';
import { MAX_NETWORK_POINTS, MAX_NETWORK_RANGES, resolveNetworkIntent, networkSelectionState, type NetworkIntent } from '../../src/domain/networkSelection.ts';
...
export type { NetworkIntent } from '../../src/domain/networkSelection.ts';
...
export function validNetworkIntent(value: unknown): value is NetworkIntent {
  ...
  const path = (value: unknown): value is string[] => Array.isArray(value) && value.length > 0 && value.length <= MAX_NETWORK_RANGES && value.every(id);
  if (input.expectedState !== undefined && (typeof input.expectedState !== 'string' || !/^[a-f0-9]{64}$/.test(input.expectedState))) return false;
  if (input.via !== undefined || input.paths !== undefined) {
    if (!Array.isArray(input.via) || input.via.length > MAX_NETWORK_POINTS - 2 || !input.via.every(anchor) || !Array.isArray(input.paths) || input.paths.length !== input.via.length + 1 || !input.paths.every(path) || input.paths.flat().length > MAX_NETWORK_RANGES || JSON.stringify(input.paths.flat()) !== JSON.stringify(input.selectedPath)) return false;
  }
  return id(input.id) && id(input.areaId) && id(input.generation) && anchor(input.start) && anchor(input.end) && path(input.selectedPath) && ['open','completed','later','not-deliverable'].includes(input.status);
}
...
export async function handleNetworkIntent(request: Request, db: D1DatabaseLike, campaignId: string, access: AccessContext, notify?: () => Promise<unknown>) {
  ...
  const intent = input;
  const fingerprint = JSON.stringify({ areaId:intent.areaId,generation:intent.generation,start:intent.start,end:intent.end,selectedPath:intent.selectedPath,status:intent.status,via:intent.via,paths:intent.paths,expectedState:intent.expectedState });
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (!intent.expectedState) return response(409,'network_selection_upgrade_required');
    ...
    try {
      ...
      if (await networkSelectionState(tasks,route.ranges) !== intent.expectedState) return response(409,'network_work_changed');

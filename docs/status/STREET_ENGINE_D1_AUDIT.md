...
Status: Phase B, local fix checkpoint; live attribution and real-device acceptance remain blocked on current device/Cloudflare evidence. Recorded 2026-09-12 UTC. No staging deployment for the Phase B changes yet.
...
Command: `node --experimental-transform-types --test tests/streetEngineD1Audit.test.ts`. Seven tests passed. The original results below are the Phase A baseline. Fixtures use an in-memory SQLite database with repository migrations and the installed RxDB implementation. No external requests.
...
The local fix candidate now has seven passing audit characterizations plus a real React workspace-poller test:
...
4. Repeat 0/2k/10k legacy and chunk-backed fixtures for single-field changes and no-op/replay writes. Retain the missing collection member index as a separately reviewed local migration proposal; no remote migration is authorized.

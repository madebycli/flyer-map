...
- Published branch head: `8b1bec21a0fbe0e4ca26c297cb61169df79dde3b`.
- Previous published runtime checkpoint: `987ed4a06b5e8b23d6fc6cc502c72868fca44a0d`.
- Remote CI for the current head: run `34716226826`, job `103613840553`,
  successful. The job passed the complete test suite (875 tests), TypeScript
  typecheck, dependency audit and production build.
- Local full test run: 874/875 passed. The one failure is the known
  `broadcast-channel` multi-tab Unix-socket `EPERM` in this sandbox. The
  focused D1/StreetEngine/Recovery/Sync suite is fully green.
- Local Build and dependency audit are green. Local TypeScript cannot start in
  this sandbox because the native TypeScript VFS cannot resolve
  `/proc/self/exe`; the remote typecheck is the authoritative result.
- The current code checkpoint has not been merged, deployed to Admin Staging
  or applied to any remote D1 database.

## Verified baseline

- Repository: `madebycli/flyer-map`, draft PR #92, branch `fix/street-engine-smart-marking`.
- Audited baseline runtime: `d37bda92ff9c0a21e07982a0e3e0b31f8f2fc320`.
...
Command: `node --experimental-transform-types --test tests/streetEngineD1Audit.test.ts`. The historical Phase A run had seven tests; the current audit suite has eight, including the prepared-Area delete budget regression. Fixtures use an in-memory SQLite database with repository migrations and the installed RxDB implementation. No external requests.

| Scenario | Measured result | Interpretation |
| --- | --- | --- |
| One Team rename, 2,000 unrelated legacy Houses | 36 statements, 6,056 returned rows, 6,006 snapshot rows | `handleRxdbPush` loaded the whole campaign before and after `handleCampaignMutation`, which loaded it again. Three full House reads for one Team field change. |
| Begin preparation / identical repeat, same fixture | 2,056 / 2,057 returned rows; identical generation | Begin unnecessarily loaded the whole campaign both times. Repeat did not create a new generation; duplicate upstream work is a separate lease test. |
...
| Actual RxDB, 101 feed entries changing one Team, batch size 100 | One pull of 100 documents to checkpoint 100, then one document to checkpoint 101; local name Version 101 | Confirmed early-stop mismatch: SQL limits physical feed entries then deduplicates them. Installed RxDB stopped when resulting documents were fewer than batch size. |
| Aliased `COUNT(*)` full scan over 2,000 Houses | EXPLAIN `SCAN h`, old estimator reports 1 | Estimator counted the aggregate result and missed the alias. Existing read-budget numbers could not exclude actual scans. |
...
## Phase B local fix checkpoint

The local fix candidate now has eight passing audit characterizations plus a real React workspace-poller test:

- Team and campaign metadata mutations use projected snapshots. Area metadata retains Teams/Areas, and destructive or task mutations retain the full canonical path. The 2,000-House Team fixture now reports 52 returned rows and 3 snapshot rows, with no full House read.
- Preparation begin reads one canonical Area and keeps the existing generation lease. The same fixture reports 48 and 49 returned rows for the first and repeated begin.
- Incremental RxDB pulls retain the physical page when repeated changes would deduplicate below the requested batch size. Actual RxDB reaches the feed head without a second external trigger, while ordinary compact pages still return unique documents.
- Workspace preparation status reads pause while offline or hidden, stop retrying authorization failures, and retry transient failures with a bounded exponential delay. The hook test covers six failed reads and the 2s, 4s, 8s, 16s and 30s schedule.
- The local D1 read estimator maps `SCAN h` back to `house_tasks` and reports the 2,000-row scan.

These are local behavior and cost-boundary fixes. They do not establish the real preparation failure, Cloudflare rows-read attribution or visible iPad geometry. The collection member index remains documented only; no remote migration is authorized.

## Confirmed Area-delete read amplification and fix

Master's additional evidence narrowed one concrete amplification path: after
deleting a large Area whose StreetEngine data had worked, the operation used a
large amount of D1 Rows Read and ended with an Area sync error. Before the new
checkpoint, the Area RxDB push loaded a projected Area snapshot, derived
`area.delete`, and then `handleCampaignMutation` loaded the complete campaign
snapshot again. That full snapshot included every legacy/physical Street and
House row. Only later, when building the feed, `chunkAreaDelete` replaced the
task arrays with empty arrays for a prepared Base-Storage Area. The expensive
read had therefore already happened even though the feed itself was compact.

The fix in `8b1bec21` detects the target's `street_base_areas` manifest before
loading the mutation snapshot. For a prepared Area it omits both task
collections entirely and publishes the Area tombstone; the existing client
materialization filters child documents through the now-missing Area, so no
large child-tombstone batch is required. If the Area is legacy or not present
in Base Storage, the snapshot is limited to that Area so its child tombstones
remain available. Authorization behavior for scoped editors is unchanged.

The regression test uses 2,000 House rows plus a prepared-Area manifest. It
measures 5 snapshot rows and 55 returned rows, then confirms the Area and all
House rows are removed. These are local SQLite budget estimates, not a
Cloudflare billing measurement, but they reproduce the previous unbounded
read shape and prove the new branch avoids it.

## Additional confirmed evidence

The audit suite now has eight passing characterizations:
...
| Preparation POST/retry, `areaTaskPreparation.ts` | Canonical Area by campaign/Area plus point state lookup | Begin no longer loads unrelated campaign task rows. Identical repeated begin retains generation; duplicate upstream work is a separate lease question. |
| Preparation status, `useNetworkWorkspace.tsx`, `areaTaskPreparationApi.ts` | Area by ID/campaign; preparation by composite PK; job by campaign/Area; schema probes | Initial read for every permitted Area. Pending states use the existing 2-second cadence. Hidden/offline tabs pause, 401/403 stop, and transient failures use a bounded exponential retry. The effect no longer restarts on the POST's local `preparing` state. Live frequency is not yet measured. |
...
| Incremental pull, `rxdbSync.ts` | `(campaign_id,collection_name,seq)` or `(campaign_id,scope_team_id,collection_name,seq)` range and LIMIT | At most requested physical feed rows. When repeated changes deduplicate below a full page, the physical page is retained so RxDB continues to the next cursor; ordinary compact pages remain deduplicated. Every pull also reads generation manifests and revision. Bootstrap and compact expansion still need live size measurement. |
...
| User Team/Area/Street push, `rxdbSync.handleRxdbPush`, `mutationHandler.ts` | Collection-specific projected snapshot for campaign/Team/Area metadata; full snapshot for task, destructive and scoped operations | Team/campaign metadata writes no longer load unrelated task rows. Destructive/task operations retain the full canonical validation path. `campaignExecution` splits batches into one-row DO calls to preserve invocation budget; this does not reduce total reads. Client batch size 20, server accepts up to 40. |
...
2. Keep the local preparation polling and snapshot projection fixes under CI, then verify the same behavior on the authorized staging runtime. Never restart a server generation to fetch status.
3. Keep the local replication continuation contract under CI. Physical feed pages preserve RxDB's full-page continuation, compact rows remain bounded by the physical limit, and deletion/scope order and checkpoints remain unchanged.
4. Repeat 0/2k/10k legacy and chunk-backed fixtures for single-field changes, Area deletion and no-op/replay writes. The prepared Area-delete path is now bounded before snapshot materialization; retain the missing collection member index as a separately reviewed local migration proposal; no remote migration is authorized.

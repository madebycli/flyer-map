...
test("Stats uses the real production launcher graph and stays bounded/offline-readable", async () => {
  const [worker, api, shell, hub, css, domain] = await Promise.all([
  ...
  assert.match(shell, /<StatisticsHub/u);
  assert.match(shell, /statisticsOpen/u);

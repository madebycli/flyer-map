...
export class NetworkD1 implements D1DatabaseLike {
  ...
  constructor(baseChunks=false,fullSchema=false){this.sqlite.exec('PRAGMA foreign_keys=ON');for(const file of fullSchema?readdirSync(new URL('../../migrations/',import.meta.url)).filter(name=>name.endsWith('.sql')&&(baseChunks||!name.startsWith('0023_'))).sort():['0001_initial.sql','0002_m4_access.sql','0003_m5_mutations.sql','0004_m6_task_source_provenance.sql','0005_m6_house_tasks.sql','0014_auto_area_task_preparation.sql','0017_rxdb_sync_changes.sql','0022_street_house_network.sql',...(baseChunks?['0023_street_base_chunks.sql']:[])])this.sqlite.exec(readFileSync(new URL('../../migrations/'+file,import.meta.url),'utf8'));}

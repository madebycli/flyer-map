...
test("push success is only acknowledged when the post-commit master matches the requested business state", async () => {
  ...
  assert.match(worker, /const canonical = await loadCampaignSnapshot\(db, campaignId, \{[^\n]+\}\);\s*const master = currentDocument\(canonical, collectionName, next\.id, next\);\s*if \(sameBusinessDocument\(master, next\)\) continue;/u);

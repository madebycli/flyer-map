...
test("Campaign and Team text/color bursts trail into one upstream window", async () => {
  ...
  assert.match(app, /function TeamNameInput\(/u);
  assert.match(app, /const \[draft, setDraft\] = useState\(name\);/u);
  assert.match(app, /onChange=\{\(event\) => setDraft\(event\.target\.value\)\}/u);
  assert.match(app, /onBlur=\{commit\}/u);
  assert.match(app, /window\.setTimeout\(flushRxdbDrafts, 0\)/u);
  assert.match(settings, /const \[campaignNameDraft, setCampaignNameDraft\] = useState\(campaign\.name\);/u);
  assert.match(settings, /onChange=\{\(event\) => setCampaignNameDraft\(event\.target\.value\)\}/u);
  assert.match(settings, /onBlur=\{commitCampaignNameDraft\}/u);
  assert.match(settings, /window\.setTimeout\(onCommitCampaignDraft, 0\)/u);
